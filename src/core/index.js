...
import type { WS, Settings, WSPath, EmitEventParams, ProtectedProps } from '../types'
...
export default class Core {
  ...
  wsOn: (event: string, listener: (...params: any) => mixed) => void
  createMock: (ws: WS, isInner?: boolean) => void

  constructor(ws?: WS, settings: Settings = defaultSettings, cb: (pp: ProtectedProps) => void) {
  ...
  onOpen = (cb: (e: any) => mixed) => {
    ...
    if (this.#isMock || this.ws instanceof Core || this.#settings.mock) {
      this.#mockCallbacks.onOpen = cb
    }
  }

  onError = (cb: (e: any | NodeWS.ErrorEvent) => mixed) => {
    if (typeof this.ws.on === 'function') {
      this.#wsOn('error', (e) => {
        this.#browserReconnect(e.code, true)
        cb(e)
  ...
  onClose = (cb: (e: CloseEvent | NodeWS.CloseEvent) => void) => {
    if (typeof this.ws.on === 'function') {
      this.#wsOn('close', (e) => {
  ...
  onMessage = ({ shouldParseJSON }: { shouldParseJSON?: boolean } = {}) => (
    this.#stream
  ...
  // for mock
  connect = (ws: WS, conn: {} = {}, req: {} = {}, headers: Array<string> = []) => {
    if (ws instanceof Core) {
